...
// codebase (e.g. inventory.service.ts reading catalog's
// product-variant.repository.ts). Shared here between
// wishlist.service.ts and back-in-stock-request.service.ts to avoid
// duplicating the same three checks in both files.
 ...
 *  customerId (same boundary CUST-001 itself established). Uses the
...
/** Archived (or, defensively, missing) customers cannot create or cancel
 *  wishlist/back-in-stock records — mirrors customers/address.service.ts's
 *  own assertCustomerActive exactly. Reads are never gated by this check. */
 ...
 * trusted from a syntactically valid ObjectId alone. Mirrors
 * product-variant.service.ts's own assertValidProduct exactly.
 */
 ...
 * variant+organization — "in stock" is a per-location fact (INV-001), but a
 * back-in-stock request is not location-specific (a customer doesn't pick a
 * warehouse). Reuses INV-001's existing, unmodified stock-balance.repository.ts
 * `list()` rather than adding a new aggregate function to INV-001 itself.
 * A 1000-row page is a pragmatic bound — no organization is expected to
 * have anywhere near that many locations for a single variant.
 */
