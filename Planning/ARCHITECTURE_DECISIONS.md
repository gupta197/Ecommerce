...
**Known gap, recorded rather than silently resolved**: the approved `customer.archived`/`customer.restored` audit events are **not implemented** — wiring them requires adding new entries to `AUDIT_ACTIONS`/`AUDIT_ENTITY_TYPES` in `modules/audit/models/audit-event.model.ts`, which conflicts with this task's explicit "do not modify SEC-003" boundary. This is recorded in `OPEN_DECISIONS.md` (`DEC-007`) for a follow-up decision rather than resolved unilaterally in either direction.
